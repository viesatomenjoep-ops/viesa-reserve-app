    ...
    <div className="p-8 h-full bg-slate-50">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-2">
          <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full font-medium text-sm flex items-center gap-1"><AlertCircle className="w-4 h-4" /> 3 Open</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
          <div className="bg-slate-100 p-4 font-bold text-slate-700 flex justify-between">Nieuw <span className="bg-slate-200 px-2 rounded-full">1</span></div>
          <div className="p-4 flex-1 space-y-4">
            {orders.filter(o => o.status === 'new').map(order => (
              <div key={order.id} className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <button className="w-full bg-amber-500 hover:bg-amber-600 text-white font-medium py-2 rounded-lg transition-colors">Start Bereiding</button>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* In Bereiding */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
          <div className="bg-slate-100 p-4 font-bold text-slate-700 flex justify-between">Maken <span className="bg-slate-200 px-2 rounded-full">1</span></div>
          <div className="p-4 flex-1 space-y-4">
             {orders.filter(o => o.status === 'preparing').map(order => (
              <div key={order.id} className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                ...
                <button className="w-full bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 rounded-lg transition-colors">Klaar (Runner ophalen)</button>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Klaar voor Runner */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
          <div className="bg-slate-100 p-4 font-bold text-slate-700 flex justify-between">Serveren <span className="bg-slate-200 px-2 rounded-full">1</span></div>
          <div className="p-4 flex-1 space-y-4">
            {orders.filter(o => o.status === 'ready').map(order => (
              <div key={order.id} className="bg-green-50 border border-green-200 rounded-xl p-4">
                ...
                <button className="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-2 rounded-lg transition-colors flex justify-center items-center gap-2"><Check className="w-4 h-4"/> Geleverd</button>
              </div>
            ))}
          </div>
        </div>
