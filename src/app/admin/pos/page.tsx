  ...
  return (
    <div className="p-8 min-h-screen bg-sand-gradient relative overflow-hidden">
      ...
      <div className="mb-8 relative z-10">
        <h1 className="text-3xl font-bold font-serif text-[#3d3935]">POS Dashboard</h1>
        <p className="text-stone-500">Kitchen & Bar Display (QR Code bestellingen)</p>
      </div>
            ...
            {orders.filter(o => o.status === 'new').map(order => (
              <div key={order.id} className="bg-white/60 border border-white/80 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="font-bold font-serif text-lg text-stone-900">{order.bed}</h3>
                  <span className="flex items-center gap-1 text-xs text-stone-500 bg-white/80 px-2 py-1 rounded-lg"><Clock className="w-3 h-3"/> {order.time}</span>
                </div>
                <ul className="text-sm text-stone-700 font-medium mb-4 space-y-1">
                  {order.items.map((item, i) => <li key={i}>• {item}</li>)}
                </ul>
                <button onClick={() => updateStatus(order.id, 'preparing')} className="w-full bg-stone-900 hover:bg-stone-800 text-white font-medium py-3 rounded-xl transition-all shadow-lg">Start Bereiding</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="p-4 flex-1 space-y-4 overflow-auto">
             {orders.filter(o => o.status === 'preparing').map(order => (
              <div key={order.id} className="bg-blue-50/80 border border-blue-200/50 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="font-bold font-serif text-lg text-blue-900">{order.bed}</h3>
                  <span className="flex items-center gap-1 text-xs text-blue-700 bg-white/80 px-2 py-1 rounded-lg"><Clock className="w-3 h-3"/> {order.time}</span>
                </div>
                <ul className="text-sm text-blue-800 font-medium mb-4 space-y-1">
                  {order.items.map((item, i) => <li key={i}>• {item}</li>)}
                </ul>
                <button onClick={() => updateStatus(order.id, 'ready')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 rounded-xl transition-all shadow-lg">Klaar (Runner ophalen)</button>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="p-4 flex-1 space-y-4 overflow-auto">
            {orders.filter(o => o.status === 'ready').map(order => (
              <div key={order.id} className="bg-emerald-50/80 border border-emerald-200/50 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="font-bold font-serif text-lg text-emerald-900">{order.bed}</h3>
                  <span className="flex items-center gap-1 text-xs text-emerald-700 bg-white/80 px-2 py-1 rounded-lg"><Clock className="w-3 h-3"/> {order.time}</span>
                </div>
                <ul className="text-sm text-emerald-800 font-medium mb-4 space-y-1">
                  {order.items.map((item, i) => <li key={i}>• {item}</li>)}
                </ul>
                <button onClick={() => updateStatus(order.id, 'delivered')} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-medium py-3 rounded-xl transition-all flex justify-center items-center gap-2 shadow-lg"><Check className="w-4 h-4"/> Geleverd</button>
              </div>
            ))}
          </div>
        </div>
